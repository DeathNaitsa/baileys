...
const participants = (0, WABinary_1.getBinaryNodeChildren)(child, 'participant').map(p => {
    ...
    // 1. Beste Option: phone_number vorhanden → echte WhatsApp-Nummer
    if (phone_number && phone_number.endsWith('@s.whatsapp.net')) {
        return phone_number
    }

    // 2. Wenn kein phone_number, aber jid da und gültig → Nummer extrahieren
    if (jid && jid.endsWith('@s.whatsapp.net')) {
        return jid
    }

    // 3. Nur zur Not: lid verwenden (z. B. bei Bots oder Systemkonten)
    if (lid && lid.endsWith('@lid')) {
        return lid // Vorsicht: kein echtes WhatsApp-Konto
    }

    // 4. Fallback – falls gar nichts passt
    return null;
}).filter(Boolean);
 // filtert mögliche null-Einträge raus
...
case 'add':
case 'leave': {
    const stubTypeKey = `GROUP_PARTICIPANT_${child.tag.toUpperCase()}`;
    msg.messageStubType = Types_1.WAMessageStubType[stubTypeKey];

    console.log('stubType', participantPhoneNumber);

    const participants = [participantPhoneNumber];

    // Wenn es sich um eine "remove"-Nachricht handelt und der Sender sich selbst entfernt hat, als "leave" markieren
    const isSelfRemoval =
        participants.length === 1 &&
        (0, WABinary_1.areJidsSameUser)(participants[0], participantPhoneNumber) &&
        child.tag === 'remove';
    ...
    const processNotification = async (node) => {
        ...
        switch (nodeType) {
            ...
            case 'w:gp2':
              ...
              console.log(handleEncryptNotification(node))
                break;
            case 'mediaretry':
